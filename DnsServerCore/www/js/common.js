...
function HTTPRequest(
  ...
  if (url != null && url.url != null) finalUrl = arguments[0].url;
  else finalUrl = url;

  if (method == null) method = arguments[0].method;

  if (method == null) method = "GET";

  if (data == null) {
    if (arguments[0].data == null) data = "";
    else data = arguments[0].data;
  }

  if (isTextResponse == null) isTextResponse = arguments[0].isTextResponse;

  if (isTextResponse == null) isTextResponse = false;

  var dataType = isTextResponse ? null : "json";

  if (success == null) success = arguments[0].success;

  var async = success != null;

  if (error == null) error = arguments[0].error;

  if (invalidToken == null) invalidToken = arguments[0].invalidToken;
  ...
  if (dontHideAlert == null) dontHideAlert = arguments[0].dontHideAlert;

  if (dontHideAlert == null || !dontHideAlert) hideAlert(objAlertPlaceholder);

  if (showInnerError == null) showInnerError = arguments[0].showInnerError;

  if (showInnerError == null) showInnerError = false;

  if (objLoaderPlaceholder == null)
    objLoaderPlaceholder = arguments[0].objLoaderPlaceholder;

  if (processData == null) processData = arguments[0].processData;

  if (contentType == null) contentType = arguments[0].contentType;

  if (objLoaderPlaceholder != null)
    objLoaderPlaceholder.html(
      "<div style='width: 64px; height: inherit; margin: auto;'><div style='height: inherit; display: table-cell; vertical-align: middle;'><img src='img/loader.gif'/></div></div>",
    );
  ...
  $.ajax({
    ...
    success: function (response, status, jqXHR) {
      if (objLoaderPlaceholder != null) objLoaderPlaceholder.html("");

      if (isTextResponse) {
        if (success == null) successFlag = true;
            ...
            if (invalidToken != null) invalidToken();
              ...
              showAlert(
                ...
                response.errorMessage +
                  (showInnerError && response.innerErrorMessage != null
...
function showAlert(type, title, message, objAlertPlaceholder) {
  ...
  if (objAlertPlaceholder == null) objAlertPlaceholder = $(".AlertPlaceholder");
...
function serializeTableData(table, columns, objAlertPlaceholder) {
  var data = table.find(
    'input:text, :input[type="number"], input:checkbox, input:hidden, select',
  );
  var output = "";
      ...
      if (cell.attr("type") == "checkbox") {
        ...
        if (cellValue === "" && !optional) {
          showAlert(
            "warning",
            "Missing!",
            "Please enter a valid value in the text field in focus.",
            objAlertPlaceholder,
          );
          cell.focus();
          return false;
