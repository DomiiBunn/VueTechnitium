<template>
  <div class="inset">
    <div class="flex">
      <div>
        <v-chip
          ...
          dark
        >
          <span>Type:</span><span class="chip-inset">DNAME</span>
        </v-chip>
      </div>
      <div>
    ...
    <v-text-field
      ...
      hint="Time To Leave in Seconds"
      v-model="localDetails.rData.dname"
    ></v-text-field>
...
export default {
  ...
  methods: {
    ...
    saveZone() {
      ...
      this.$axios
        .post(
          `/api/zone/updateRecord?token=${localStorage.appToken}&zone=${this.$route.params.zoneId}&type=${a.type}&domain=${this.recordDetails.name}&newDomain=${a.name}&ttl=${a.ttl}&comments=${a.comments}&dname=${a.rData.dname}`
        )
        .then(() => {
