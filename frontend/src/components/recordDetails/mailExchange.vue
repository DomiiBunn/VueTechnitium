<template>
  <div class="inset">
    <div class="flex">
      <div>
        <v-chip
          ...
          dark
        >
          <span>Type:</span><span class="chip-inset">MX</span>
        </v-chip>
      </div>
      <div>
    ...
    <v-text-field
      ...
      hint="Time To Leave in Seconds"
      v-model="localDetails.rData.preference"
    ></v-text-field>
    <v-text-field
      label="Exchange"
      prepend-inner-icon="mdi-ip"
      hint="Time To Leave in Seconds"
      v-model="localDetails.rData.exchange"
    ></v-text-field>
...
export default {
  ...
  methods: {
    ...
    saveZone() {
      ...
      this.$axios
        .post(
          `/api/zone/updateRecord?token=${localStorage.appToken}&zone=${this.$route.params.zoneId}&type=${a.type}&domain=${this.recordDetails.name}&newDomain=${a.name}&ttl=${a.ttl}&comments=${a.comments}&preference=${this.recordDetails.rData.preference}&newPreference=${a.rData.preference}&exchange=${this.recordDetails.rData.exchange}&newExchange=${a.rData.exchange}`
        )
        .then(() => {
